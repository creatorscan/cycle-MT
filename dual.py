...
def dual(args):
    if "clsp.jhu.edu" in subprocess.check_output(["hostname", "-f"]).decode():
        cvd = subprocess.check_output(["/usr/local/bin/free-gpu", "-n", str(1)]).decode().strip()
    ...
    for m in ['A', 'B']:
        ...
        #optimizers[m] = torch.optim.SGD(models[m].parameters(), lr=1e-6, momentum=0.9)
        ...
        for t in range(start, start + len(train_srcs['A'])):
            ...
            for m in ['A', 'B']:
                ...
                if show_log:
                    ...
                    print('[s]', ' '.join(sent))
                hyps = modelA.beam(sent, beam_size=10)
                ...
                for ids, smid, dist in hyps:
                    ...
                    tgt_sent_var = to_input_variable([['<s>'] + sent + ['</s>']], vocabB.tgt, cuda=args.cuda)
