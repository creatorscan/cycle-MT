...
class NMT(nn.Module):
    def __init__(self, args, vocab):
        ...
        self.encoder_lstm_2 = nn.LSTM(2 * args.hidden_size, args.hidden_size, bidirectional=True, dropout=args.dropout)
        self.encoder_lstm_3 = nn.LSTM(2 * args.hidden_size, args.hidden_size, bidirectional=True, dropout=args.dropout, num_layers=2)
    ...
    def encode(self, src_sents, src_sents_len):
        ...
        output, _ = self.encoder_lstm(packed_src_embed)
        import ipdb; ipdb.set_trace()
        output =  output[0][::2]
        output, _ = pad_packed_sequence(output)
        output = self.encoder_lstm_2(output)
        output =  output[0][::2]
        output, _ = pad_packed_sequence(output)
        output, (last_state, last_cell) = self.encoder_lstm_3(output)
        output, _ = pad_packed_sequence(output)
